    ...
    public InfoTowerHUD(final TroyTD game, final Stage stage, Map map, float topHUDHeight,
        ...
        upgradeEnemyAmountButton.addListener(new ChangeListener() {
            ...
            public void changed(ChangeEvent event, Actor actor) {
                if (gameScreen.money < (towerPlace.getTower()
                        .getStat("enemyAmount")
                        .getLevel() + 1) * 0.4 * Tower.upgradeCost + Tower.upgradeCost * 2 + Tower.upgradeCost) return;
                gameScreen.money -= (towerPlace.getTower()
                        .getStat("enemyAmount")
                        .getLevel() + 1) * 0.4 * Tower.upgradeCost + Tower.upgradeCost * 2;
                towerPlace.getTower()
        ...
        upgradeUnitAmountButton.addListener(new ChangeListener() {
            ...
            public void changed(ChangeEvent event, Actor actor) {
                if (gameScreen.money < (towerPlace.getTower()
                        .getStat("unitAmount")
                        .getLevel() + 1) * 0.4 * Tower.upgradeCost + Tower.upgradeCost * 2) return;
                gameScreen.money -= (towerPlace.getTower()
                        .getStat("unitAmount")
                        .getLevel() + 1) * 0.4 * Tower.upgradeCost + Tower.upgradeCost * 2;
                towerPlace.getTower()
